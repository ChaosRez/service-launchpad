...
- `vmagent` scrapes `fastapi-service`, the local host control-plane via `host.minikube.internal:8080`, `kube-state-metrics`, `VictoriaMetrics`, and `Mimir`, then remote-writes the resulting series to both metric stores
