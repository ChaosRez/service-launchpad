...
- `load-test-fastapi-service.sh`: runs an in-cluster `k6` Job against `fastapi-service` and remote-writes `k6_*` metrics to `VictoriaMetrics`
...
- streams `k6` metrics to `VictoriaMetrics` at `/api/v1/write`
...
Implementation note: the script uses `k6`'s `experimental-prometheus-rw` output, which is the official `k6` path for streaming metrics into a Prometheus-compatible backend such as `VictoriaMetrics`.
